...
namespace ark4ccma.Controllers
{
  ...
  public class DocumentController : Controller
  {
    ...
    public dynamic GetDocument(string unidDoc)
    {
      dynamic doc = _connection.Query<dynamic>(@"SELECT d.[Id],d.[DocumentTableID],d.[ApplicationNumber],d.[FormName]
      ,d.[FieldName]
      ,d.[FieldValue]
  ...
  Where t.DocumentUNID = @docunid",
    ...
    public dynamic GetApplicationDocuments(string app)
    {
      return _connection.Query<dynamic>("select Id,ApplicationNumber,DocumentUNID,FormName from ark.DocumentTable where  ApplicationNumber = @appNo",new {appNo = app })
        .ToList<dynamic>();
    }

    [HttpGet("application/{appno}")]
    public dynamic GetApplication(string appno)
    {
      dynamic appData = _connection.Query<dynamic>(@"SELECT d.[Id],d.[DocumentTableID],d.[ApplicationNumber],d.[FormName]
      ,d.[FieldName]
      ,d.[FieldValue]
      FROM [ark].[DocumentDataTable] d inner join ark.DocumentTable t on d.DocumentTableID = t.Id
    ...
    [HttpGet("search/{term}")]
    public List<dynamic> GetSearchResults(string term)
    {
...
SELECT top 100 [ApplicationNumber]
  ...
  group by ApplicationNumber,FormName
  order by 3 desc, 1 desc", new {SearchTerm = $"%{term}%" }).ToList<dynamic>();
  }
  }
}